...
class NeuralCompressor:
    ...
    def compression_progress(self, spacetime: np.ndarray, window: int = 50) -> List[float]:
        """
        ...
        for t in range(rows - 1):
            ...
            # Predict
            output = self.model(input_row) # We use RNN to predict next *token* in sequence or next *row*?
