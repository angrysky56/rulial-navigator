...
from .server.rpc import start_server, probe_rule, ProbeRequest
...
def navigate(start_rule: int = 110, steps: int = 20):
    ...
    console.print(f"[bold green]Starting Navigation Swarm at Rule {current_rule}[/bold green]")
    ...
    for i in range(steps):
        ...
        for p in probes:
            style = "white"
            if p.rule == best_rule: style = "bold green"
            if p.wolfram_class == 4: style = "bold gold1"
        ...
        if best_rule == current_rule:
            ...
            console.print(f"[cyan]Gradient found! Moving to Rule {best_rule} (Mag: {magnitude:.4f})[/cyan]")
        if best_rule == current_rule:
            console.print("[yellow]Local optimum reached. Swarm staying put.[/yellow]")
        else:
            console.print(f"[cyan]Gradient found! Moving to Rule {best_rule} (Mag: {magnitude:.4f})[/cyan]")
...
def probe_2d(
    ...
    seed: int = typer.Option(224, help="Initial Rule Code (Default: Game of Life 224/std is actually complex mapping, try simple ints)"),
    epochs: int = typer.Option(1000, help="Max epochs to run"),
    window: int = typer.Option(24, help="Observer Window Size (Quantum Analysis). Warning: Complexity scales exponentially.")
):
    """
    ...
    runner = RulialProbe2D(width=width, height=height, steps=steps, seed_rule=seed, obs_window=window)
